...
void Generate_Key_Pair(BIGNUM *p, BIGNUM *q, BIGNUM *g, BIGNUM **private_key,
  ...
  // Determine number of bytes needed to represent q
  int q_bytes = BN_num_bytes(q);
  if (q_bytes <= 0)
    q_bytes = 1;
  size_t prng_out_len = (size_t)q_bytes;

  // Prepare a seed copy and shuffle/alter slightly using seed_offset so that
  // Alice and Bob would be assigned the different private keys. (Alice and bob
  // would have different offsets)
  unsigned char seed_copy[32];
  memcpy(seed_copy, seed, 32);
  // Simple alteration: XOR final byte with seed_offset
  seed_copy[31] =
  ...
  // Convert random bytes to BIGNUM
  BIGNUM *k = BN_bin2bn(random_bytes, (int)prng_out_len, NULL);

  // private_candidate = k mod q  (in range 0..q-1)
  BIGNUM *private_candidate = BN_new();
  BN_mod(private_candidate, k, q, ctx);

  // To ensure private key is in {1, ..., q-1}, if result == 0 set to 1
  if (BN_is_zero(private_candidate)) {
    BN_set_word(private_candidate, 1);
  }

  // Assign the private key to output (transfer ownership)
  *private_key = private_candidate;

  // Compute public_key = g^private mod p
  BIGNUM *pub = BN_new();

  // BN_mod_exp(pub, base, exp, mod, ctx) computes base^exp mod mod
  BN_mod_exp(pub, g, *private_key, p, ctx);
