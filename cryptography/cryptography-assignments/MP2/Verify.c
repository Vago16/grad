...
int Unsigncrypt(BIGNUM *p, BIGNUM *q, BIGNUM *g, BIGNUM *r, BIGNUM *s,
  ...
  BIGNUM *exp = NULL;
  BIGNUM *r_mod_q = NULL;
  BIGNUM *base_right = NULL;
  BIGNUM *base = NULL;
  int verification_result = 0;

  // Compute w = (g * y_a^r)^(s * x_b) mod p
  // -> Compute exp = s * x_b mod q
  exp = BN_new();
  BN_mod_mul(exp, s, bob_private_key, q, ctx);

  // -> Reduce r mod q for modular exponentiation
  r_mod_q = BN_new();
  BN_mod(r_mod_q, r, q, ctx);

  // -> Compute y_a^r mod p
  base_right = BN_new();
  BN_mod_exp(base_right, alice_public_key, r_mod_q, p, ctx);

  // -> Compute base = g * base_right mod p
  base = BN_new();
  BN_mod_mul(base, g, base_right, p, ctx);

  // -> Compute w = base^exp mod p
  BN_mod_exp(w, base, exp, p, ctx);

  // Derive key k = G(w)
  Hash_G(w, key, 16);

  // Decrypt the message m = D_k(c)
  int decrypt_ret =
      Decrypt_Message(key, ciphertext, ciphertext_len, message, message_len);
  if (!decrypt_ret) {
  ...
  // Verify that r' = H(m, w)
  Hash_H(*message, *message_len, w, r_prime);

  // Compare r' with the received r
  verification_result = BN_cmp(r, r_prime) == 0;
