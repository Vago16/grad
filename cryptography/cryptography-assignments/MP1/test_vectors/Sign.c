...
/*============================
  Signcrypt
    ...
    • message: The plaintext message to be signcrypted
    • message len: Length of the message
    • alice private key: Alice’s private key (xa)
    • bob public key: Bob’s public key (yb)
    • r, s: Output parameters for the signature components
    • ciphertext: Output parameter for the encrypted message
    • ciphertext len: Output parameter for the length of the ciphertext
    • seed: Seed value for random number generation
    • seed len: Length of the seed
    • seed offset: Offset value for randomness generation
==============================*/
void Signcrypt(BIGNUM *p, BIGNUM *q, BIGNUM *g, const unsigned char *message,
               size_t message_len, BIGNUM *alice_private_key,
               BIGNUM *bob_public_key, BIGNUM **r, BIGNUM **s,
               unsigned char **ciphertext, size_t *ciphertext_len,
               unsigned char *seed, size_t seed_len, int seed_offset) {
  // 6. Compute s = x/(r + x_a) mod q ; s = x/(1 + x_a * r) mod q
  // 7. Compute c = E_k(m) (encrypt the message)
  BN_CTX *ctx = BN_CTX_new();
  if (!ctx)
    return;

  // 1. Generate random k using PRNG with modified seed
  unsigned char seed_mod[32];
  if (seed_len >= 32) {
    memcpy(seed_mod, seed, 32);
  } else {
    memset(seed_mod, 0, 32);
    if (seed_len > 0)
      memcpy(seed_mod, seed, seed_len);
  }
  seed_mod[31] = (unsigned char)seed_offset;

  unsigned char *rand_bytes = PRNG(seed_mod, 32, 256);
  if (!rand_bytes) {
    fprintf(stderr, "PRNG failed in Signcrypt\n");
    BN_CTX_free(ctx);
    return;
  }
  BIGNUM *k = BN_bin2bn(rand_bytes, 256, NULL);
  free(rand_bytes);
  if (!k) {
    BN_CTX_free(ctx);
    return;
  ...
  BN_mod(k, k, q_minus_one, ctx);
  BN_add(k, k, one); /* ensure in [1, q-1] */

  // 2. Compute w = y_b^k mod p (shared secret)
  BIGNUM *w = BN_new();
  if (!BN_mod_exp(w, bob_public_key, k, p, ctx)) {
    fprintf(stderr, "BN_mod_exp failed for w\n");
    goto sign_cleanup;
  }

  // 3. Derive symmetric encryption key K_e = Hash_G(w)
  unsigned char key[16]; /* Hash_G truncates to 16 bytes in your project */
  Hash_G(w, key, sizeof(key));

  // 4. Encrypt the message using K_e
  Encrypt_Message(key, message, message_len, ciphertext, ciphertext_len, seed,
                  seed_len);

  // 5. Compute r = Hash_H(m, w)
  BIGNUM *r_bn = BN_new();
  Hash_H(message, message_len, w, r_bn);

  // 6) Compute s = k/(r + x_a) mod q
  //    which can be expressed as s = k * (r + x_a)^{-1} mod q
  BIGNUM *sum = BN_new();
  BN_mod_add(sum, r_bn, alice_private_key, q, ctx); /* sum = (r + x_a) mod q */

  BIGNUM *inv =
      BN_mod_inverse(NULL, sum, q, ctx); /* inverse of (r + x_a) mod q */
  if (!inv) {
    fprintf(stderr, "BN_mod_inverse failed in Signcrypt\n");
    BN_free(r_bn);
    goto sign_cleanup;
  }

  BIGNUM *s_bn = BN_new();
  BN_mod_mul(s_bn, k, inv, q, ctx); /* s = k * inv mod q */
