...
import itertools    #let comparison of state objects not happen directly, as explained in https://www.geeksforgeeks.org/python/heapq-with-custom-predicate-in-python/
...
def astar(initial_state, given_map, terrain_costs):
    ...
    '''
    #counter ensures not tuples are seen as equal, as is common with heapq
    counter = itertools.count()
    #initialize empty list for the priority queue to be the frontier(the open list)
    open_list = []
    #heapq.heappush method takes two args
    heapq.heappush(open_list, (heuristic.heuristic(initial_state), 0, next(counter), initial_state, None))
    ...
    while open_list:
        ...
        #_ means ignore the counter when popping
        f, g, _, curr, parent = heapq.heappop(open_list)
        ...
        for neighbor, cost in state.get_neighbor(curr, given_map, terrain_costs):
            ...
            heapq.heappush(open_list, (n_f, n_g, next(counter), neighbor, curr))
